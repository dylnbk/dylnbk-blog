...
time Skype, YouTube, Facebook and Twitter launched; the shift to web 2.0 had truly begun (Ortiz-Ospina, Roser and Ritchie, 2018).
...
capitalizations in the world (Armstrong, 2021). <br><br>
...
data that was collected without users’ consent (Frankenfield, 2021). In 2017, Equifax suffered a large scale data breach exposing the
private records of roughly 150 million citizens across the world (The Guardian, 2019). As of 2020, the value of the data economy
between the EU and UK exceeded 400 billion euros and is projected to more than double by 2025 (Statista, 2022). Securing the rights
...
Blockchains that enable services like Bitcoin are dependent on substantial amounts of energy and have frequently been in the news for the large electricity demands needed to ‘mine’ coins (Huang, O’Neill and Tabuchi, 2021), but what does mining Bitcoin actually mean? <br><br>
...
This leads us to the next set of major differences between various types of blockchains, the ‘consensus mechanism’. To solve the problem of trust in a decentralised network, various approaches have been taken. Proof of Work (PoW), Proof of Stake (PoS), Delegated Proof of Stake (DPoS), Practical Byzantine Fault Tolerance (PBFT) and Directed Acyclic Graph (DAG) are currently the most commonly used algorithms (Guo and Yu, 2022). <br><br>
...
programmers write scripts that can be stored and executed by the blockchain itself (Buterin, 2013). This had a large impact on the
...
fees instead of newly minted coins (Buterin, 2013). <br><br>
...
electricity needs as a German household would use in weeks, even months (Sedlmeir et al., 2020). Given blockchains such as Ethereum
...
achieve more than 4000 transactions per second (Zhou et al., 2020). It can take 30 minutes for a transaction to be considered fully
...
the blockchain community, the Ethereum developers decided to ‘roll back’ the chain and fork the project after patching the bug (Shin,
2022). This affects trust in the system, which as revealed by the survey is already quite low. With only 25 out of 92 respondents stating
